...
const els = {
  ...
  mapHead: $("mapHead"), mapRoundel: $("mapRoundel"), mapNow: $("mapNow"), mapDir: $("mapDir"), mapRival: $("mapRival"),
  mapView: $("mapView"),
...
function startGame(line, pkRace) {
  ...
  els.mapRival.classList.toggle("hidden", !battle);
  if (battle) {
...
function updateLeadChip() {
  ...
  // 對手位置膠囊（地圖標題列）：站名放大才一眼看得到對手在哪
  els.mapRival.innerHTML =
    `<i class="rv-dot"></i><span class="rv-name">${esc(state.rivalDef.name)}</span>${rivalStation().zh}`;
}
...
const pk = {
  ws: null, role: null, code: null,
  key: null,          // 重連憑證：列車長=hostKey，隊友=上車時伺服器發的 guestKey
  retries: 0, reconnectTimer: 0,
  ...
  oppCps: 0,          // 對手手速估計（字元/秒，觀戰畫面用）
  lastPAt: 0, lastPChars: 0, // 上一筆進度的時刻與字數（算手速用）
...
/* re=true 為斷線重連：保留賽況狀態、不重置畫面，靜靜接回去 */
function pkConnect(re) {
  pk.closing = false;
  clearTimeout(pk.reconnectTimer);
  if (!re) {
...
function pkHandle(msg) {
  if (msg.t === "key") { pk.key = msg.key; return; } // 上車發的重連憑證
  if (msg.t === "sync") { // 重連後的賽況快照
    if (msg.s === "racing") {
      pk.oppChars = Math.max(pk.oppChars, msg.c | 0);
      if (msg.fin && msg.fin !== pk.role) pk.oppFin = true;
      pkSendProgress(true); // DO 可能重啟過，把自己的累計進度補報回去
    } else if (msg.s === "done") {
      pkHandle({ t: "end", winner: msg.winner, reason: msg.reason }); // 離線期間已結算
    }
    return;
  }
  if (msg.t === "drop") { showPkToast("對手連線不穩，等待重連…", true); return; }
  if (msg.t === "back") { showPkToast("對手回來了！", true); return; }
  if (msg.t === "room") {
  ...
  if (msg.t === "p") {
    // 順便估手速（指數平滑）：觀戰畫面的 KPM 與場景速度用
    const now = performance.now();
    if (pk.lastPAt && msg.c > pk.lastPChars) {
      const inst = (msg.c - pk.lastPChars) / Math.max((now - pk.lastPAt) / 1000, 0.05);
      pk.oppCps = pk.oppCps * 0.6 + inst * 0.4;
    }
    pk.lastPAt = now;
    pk.lastPChars = msg.c;
    pk.oppChars = msg.c;
...
/* 對手 live 打字視圖：由累計字元數還原「正在打哪個字、打到第幾個字元」。
   只有打對的字元會回報，所以看到的是乾淨進度（不含對方的誤擊）。 */
let specLastChars = -1;

function updateSpecView() {
  els.specKpm.textContent = `${Math.round(pk.oppCps * 60)} KPM`;
  const chars = Math.min(pk.oppChars, state.totalChars);
  if (chars === specLastChars) return; // 字元沒動就不重繪
  specLastChars = chars;
  // 由 cum 找目前單字：cum[wi] <= chars < cum[wi+1]
  let wi = 0;
  while (wi < state.cum.length - 2 && chars >= state.cum[wi + 1]) wi++;
  const st = state.line.stations[wi];
  const w = stationWord(st);
  const pos = Math.min(chars - state.cum[wi], w.length);
  els.specZh.textContent = st.zh;
  els.specWord.innerHTML = [...w]
    .map((ch, k) => {
      const cls = ["ch"];
      if (ch === " ") cls.push("sp");
      if (k < pos) cls.push("done");
      else if (k === pos) cls.push("cur");
      return `<span class="${cls.join(" ")}">${ch === " " ? "&nbsp;" : ch}</span>`;
    })
    .join("");
}
...
function pkLeave() {
  clearTimeout(pk.sendTimer);
  clearTimeout(pk.reconnectTimer);
  pk.sendTimer = pk.reconnectTimer = 0;
  if (pk.ws) {
    pk.closing = true;
    try {
      // 主動下車：先告知伺服器（立即判定，對手不用等斷線寬限）
      if (pk.state === "racing" && pk.ws.readyState === 1) pk.ws.send(JSON.stringify({ t: "bye" }));
      pk.ws.close(1000);
    } catch {}
  }
  pk.ws = null;
...
function pkClosed() {
  ...
  // 斷線自動重連：部署切換或行動網路瞬斷都先接回（伺服器有 25 秒寬限期）
  // 退避 0.4/0.8/1.6/3.2/5/5 秒，共約 16 秒；沒 key 的（連線瞬間就斷）走原本流程
  if ((was === "waiting" || was === "racing" || was === "done") && pk.key && pk.retries < 6) {
    pk.retries += 1;
    if (pk.retries === 1) showPkToast("連線中斷，重新連線中…", true);
    pk.reconnectTimer = setTimeout(() => pkConnect(true), Math.min(400 * 2 ** (pk.retries - 1), 5000));
    return;
  }
...
function tick(now) {
  ...
  if (state.playing || state.finished) {
    ...
    // 觀戰：場景速度跟著對手的手速跑，像坐在對方車上看他打
    if (now - pk.lastPAt > 1500) pk.oppCps *= 0.97; // 對手停手，緩緩滑行減速
    const fullCps = state.lang === "zh" ? 2.2 : 6;
    const target = 0.18 + Math.min(pk.oppCps / fullCps, 1) * (MAX_SPEED - 0.2);
    speed += (target - speed) * 0.06;
  } else {
    speed += (0 - speed) * 0.05;
  }
  ...
  if (state.mode === "battle" && (state.playing || state.spectating) && !state.finished) {
    ...
    if (state.spectating && frame % 6 === 0) updateSpecView(); // 對手 live 打字視圖
